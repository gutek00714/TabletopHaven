...
function transformGameData(gameData) {
    ...
    // This is a sample transformation, adjust according to your schema
    return {
        name: gameData.name.find(n => n.$.primary === 'true')._,
        publisher: gameData.boardgamepublisher.map(pub => pub._),
...
async function addGameToDatabase(game) {
    try {
        await db.none('INSERT INTO games (name, publisher, categories, rating, play_time, age, foreign_names, bgg_id) VALUES (${name}, ${publisher}, ${categories}, ${rating}, ${play_time}, ${age}, ${foreign_names}, ${bgg_id})', game);
